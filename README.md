...
### 关于src下的assets(static)文件夹, 是存放静态资源的文件夹
    这里面放的东西是大家都在用的
### 如何在非路由组件中使用路由组件特有的api
    react-router-dom提供了withRouter
    withRouter是一个高阶组件, 接收一个非路由组件
                             加工生成路由组件(包含history,location,match)
### 关于assets和public文件夹
    react脚手架的设计是: src文件夹: 是源代码(程序员所有写的代码在这块)
                        写代码时可以随意引入一些资源,但是不要超过src这个范围
    所以我们最好把编程时用到的文件还有其他东西放在assets文件夹(最好不要超出src范围)
### antd中的defaultSelectedKeys只能设置一次
    后来设置的那些次都会失效
### 处理登录后导航中不默认选中主页的导航项的问题:
    有两种解决方案:
      1. 把defaultSelectedKeys改为 selectedKeys (最优)
      2.在check.js中,如果检查isLogin为true,直接去/admin/home 
        但是如果用户自己在浏览器地址栏输入/admin,还是会出现问题
### 往redux中存标题有两个场景:
    1.点击导航项,往redux中存标题
    2.页面一刷新,拿着浏览器地址栏最后的key去进行计算然后存进去(在LeftNav的挂载钩子中)
### 有如下需求: 
    用户退出登录之前,页面中导航区选中的是商品导航,用户再次登录时,还能选中商品,
    (记录你上次的位置)这个需求不能靠redux记,也不能靠local记,因为local中用户可能去删,
    应该把这个信息存到后端数据库,在用户登录一瞬间,页面发送请求时,服务器给你返回数据时多返回一个
    上次的位置
### antd中的卡片布局
    卡片头,卡片体
### antd中的表格的高级列
### token的特点
    1.token一定是服务器在你登录成功之后再生成的
    2.每一个人的token都不一样
    3.token是有有效期的(有过期时间)
    4.如果重新登录了,服务器会给你生成一个新的token,旧的token就被废掉了
